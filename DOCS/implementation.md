...
- **Admin Panel**: Administrators can view a list of website users and available products. They also have the authority to delete users or products.
...
| ProductsRemoved       | Archives removed products.               |
| CartsCategory         | Manages shopping cart containers.        |
| Carts                 | Stores individual cart items.            |
| Orders                | Tracks order status and details.         |
| Payments              | Records payment transactions.            |
| PaymentsSessions      | Manages active payment sessions.         |
...
*Note: The `ProductsRemoved` table maintains an identical structure to the `Products` table.*
...
1. Copy the product record to the `ProductsRemoved` table.
2. Delete the record from the `Products` table.

### Purchase Process

1. **Cart Creation**
   - Create a new entry in the `CartsCategory` table.
   - Add selected products to the `Carts` table.

2. **Order Initialization**
   - Create a new record in the `Orders` table linked to the cart.
   - Set the initial status to `Suspended`.

3. **Payment Processing**
   - Create records in the `Payments` and `PaymentsSessions` tables.
   - Perform an inventory check:
     - Ensure sufficient product quantities.
     - Validate the payment method.
   - **Success**:
     - Deduct inventory quantities.
     - Update the `Payments.status` to `Payed`.
     - Delete the `PaymentsSessions` record.
   - **Failure**:
     - Restore inventory quantities.
     - Set the `Payments.status` to `Cancel`.
     - Delete the `PaymentsSessions` record.
